...
export default function DrivingTime() {
  const [path, setPath] = useState("driving-time"); // default selection -> Driving Time
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setResult(null);
    // Need both locations for driving-time
    ...
    try {
      ...
      navigate("/driving-time/result", { state: { result: payload } });
      return;
    } catch (err) {
      console.error(err);
                ...
                {result && (
                  <div className="mt-4 p-4 bg-gray-50 rounded">
                    ...
                    <div className="text-sm">
                      Estimated flight time: {Math.floor(result.hours)} hour(s){" "}
                      {Math.round((result.hours % 1) * 60)} minute(s)
                    </div>
                  </div>
                )}
              </form>
